...
Versione del documento / Document version: 2026-08-03 · Applicazione / Application: 1.11.0
...
- Prima di invocare ExcelJS, un preflight isolato legge soltanto la coda ZIP, la directory centrale e le intestazioni locali. Per i workbook autorevoli limita l’archivio a 4.096 voci, 16 MiB di directory centrale, 128 MiB per singola voce espansa, 256 MiB espansi complessivi, rapporto di compressione 200:1 e nomi di voce da 1.024 byte. I limiti sono centralizzati in `appConfig.ts`; i template importati riusano lo stesso motore con il profilo più restrittivo già previsto da M15.
- Il preflight rifiuta prima dell’espansione troncamenti, multi-disk/ZIP64, cifratura, metodi o flag non supportati, voci duplicate senza distinzione tra maiuscole e minuscole, percorsi assoluti/traversali, sovrapposizioni, offset e metadati locali/centrali incoerenti, parti OOXML obbligatorie mancanti, archivi annidati, macro, fogli macro/dialogo, ActiveX, oggetti incorporati e collegamenti esterni. Gli errori IPC distinguono struttura non sicura e limite di risorse senza restituire percorso o contenuto del file.
...
Il preflight limita le classi note di espansione ZIP prima del parser ma non prova la complessità semantica di XML formalmente valido entro i limiti. È stata valutata l’esecuzione di ExcelJS in un worker terminabile: non viene introdotta in M9 perché il corpus ostile noto viene respinto con letture limitate prima del parsing e spostare migrazioni/riparazioni fuori processo aumenterebbe copie di memoria e superficie di errore. L’isolamento andrà rivalutato se fuzzing o riproduzioni locali sintetiche evidenzieranno un caso entro i limiti che blocca il main process; fino ad allora aprire file ContaMì di provenienza affidabile.
...
- L’audit npm è eseguito localmente e in CI; al 2026-08-03 riporta 0 vulnerabilità note dopo gli override compatibili di `uuid >=11.1.1` per ExcelJS e `shell-quote >=1.10.0` per la toolchain di sviluppo.
...
User strings are written as values, not interpolated into formulas. The loader never executes macros or formulas. The preflight bounds known ZIP-expansion attacks before parsing but cannot prove the semantic complexity of formally valid XML within those limits. Running ExcelJS in a terminable worker was evaluated and is not introduced in M9 because the known hostile corpus is rejected with bounded reads before parsing, while moving migrations and repairs out of process would add memory copies and failure surface. Isolation must be reconsidered if fuzzing or local synthetic reproductions find an in-budget case that blocks the main process; until then, open ContaMì files from trusted sources.
...
Implemented checks cover domain aggregation (including separate account/cash-register balances, neutral internal transfers, payment-method compatibility, utilities, condominium, vehicles, atomic and unique vehicle financing, installment classification/lifecycle, confirmed-only liquidity, and rent due/receipt allocation), future-only recurring rate changes and confirmed-history invariance, configurable-tax CRUD and constraints, investment/private-pension loss indicators and separation without double counting, reserved pension-type protection and rollover, v1–v8→v9 migration with conservative account/due-date propagation and unchanged base rates, bidirectional record synchronization and deletion, idempotent Contribution/Liquidation reconciliation with ambiguous cases, workbook round-trip, missing-workbook startup recovery, external-edit detection, validated atomic settings, strict IPC tuples, structural and round-trip verification of all eight v2 templates (catalog modes, named ranges, protected sheets, 5,000-row limit, no formulas/links, and path-redacting dialog), a fully synthetic workbook-preflight corpus for truncation, nesting, zip bombs, duplicates, traversal, encryption, ZIP64, and inconsistent local/central metadata, reproducible seeded mutations and a pre-ExcelJS rejection budget, dialog focus containment/restoration, reduced motion, a synthetic large-dataset performance budget, Node.js-baseline consistency across `engines`, direct dependencies, CI, and documentation, builds, dependency audit, reproducible Playwright UI flows in both languages/themes at 1080 px, actual `app.asar` inspection, unpacked and installed-package smoke tests with removal, independent workbook rendering, and CI rejection of private sources/workbooks/keys.
