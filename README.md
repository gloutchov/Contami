...
- Per-position investment and pension-compartment charts compare net invested capital with countervalue and then show cash-flow-adjusted monthly Modified Dietz returns between consecutive valuations. Cards include annual percentage trends, with estimates derived from consecutive closing summaries and partial periods identified. Pages, cards, and details also show initial invested capital, subsequent contributions, total liquidations, and invested/liquidated balance; **Correction** changes those summaries only and is excluded from returns.
