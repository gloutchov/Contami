...
> Stato / Status: **1.5.1 — investment cash-account reconciliation** · Licenza / License: **Apache-2.0**
...
- Grafici per ogni investimento e comparto pensione che confrontano nel tempo cifra investita e controvalore. **Aggiorna valore** è disponibile anche nella modale del singolo investimento. Ogni Versamento o Liquidazione, una tantum o periodico, richiede il conto interessato e mantiene una sola Transazione collegata come trasferimento con direzione di cassa: modifica liquidità e totalizzatori di cassa senza gonfiare redditi o spese correnti. La liquidità comprende i saldi iniziali, ignora i movimenti anteriori all’apertura del conto e completa i riferimenti mancanti soltanto quando il conto compatibile è univoco; gli altri casi vengono segnalati.
...
- Per-position investment and pension-compartment charts compare invested amount with countervalue over time. **Update value** is also available inside each investment dialog. Every one-off or recurring Contribution and Liquidation requires the affected account and keeps exactly one linked Transaction as a cash-directed transfer: it changes liquidity and cash-flow totals without inflating current income or expenses. Liquidity includes opening balances, ignores movements before the affected account opened, and fills a missing reference only when the date-compatible account is unique; all other cases are reported.
