...
> Stato / Status: **1.16.0 — riconciliazione investimenti / investment reconciliation (in sviluppo / in development)** · Licenza / License: **Apache-2.0**
...
- Grafici per ogni investimento e comparto pensione che confrontano nel tempo capitale investito e controvalore. Pagina, schede e dettagli mostrano separatamente capitale iniziale, versamenti successivi, liquidazioni totali e saldo investito/liquidato; una freccia accessibile accanto al valore segnala inoltre se l’ultima rilevazione è salita o scesa. **Aggiorna valore** registra una nuova rilevazione, mentre **Correzione** aggiunge esclusivamente una differenza ai Versamenti successivi o alle Liquidazioni, senza creare Transazioni né cambiare liquidità o controvalore. Ogni Versamento o Liquidazione reale mantiene invece una sola Transazione collegata; quando la si modifica da Transazioni presenta i normali tipi Uscita/Entrata, senza l’opzione neutra tra conti.
...
- Property records can be filtered across all twelve months and by description, and support total or per-square-metre valuations, recurring rents with separate due-period and actual-receipt dates, a paid/late/overdue/future instalment table, guided utility entries (including electricity bands), configurable taxes, optional inclusion in the common property-expense summary, yearly actuals, and date-based charts; an expense entered through **New entry** can automatically create its half-split Shared Expense.
...
- Per-position investment and pension-compartment charts compare net invested capital with countervalue over time. Pages, cards, and details separately show initial invested capital, subsequent contributions, total liquidations, and invested/liquidated balance; an accessible arrow beside the value also shows whether the latest observation rose or fell. **Update value** records an observation, while **Correction** adds a difference only to Subsequent contributions or Total liquidations, without creating Transactions or changing liquidity or countervalue. Every real Contribution or Liquidation still keeps exactly one linked Transaction; editing it from Transactions presents the standard Expense/Income choices and never the neutral-between-accounts option.
