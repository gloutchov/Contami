...
6. In **Investimenti** o **Pensione integrativa**, controlla nei quattro box capitale iniziale, versamenti successivi, liquidazioni e saldo. Le card mostrano il rendimento annuale e il dettaglio quello mensile sotto al controvalore; il primo Versamento è capitale iniziale, la linea tratteggiata indica la media e una valutazione mancante resta senza punto pur non interrompendo il tracciato. Gli immobili in affitto mostrano gli stessi periodi come rendimento locativo netto. **Correzione** non entra nel calcolo del rendimento.
...
6. In **Investments** or **Private pension**, review initial capital, subsequent contributions, liquidations, and balance in the four boxes. Cards show annual returns and details show monthly returns below countervalue; the first Contribution is opening capital, the dashed line marks the average, and a missing valuation keeps no point without breaking the plotted line. Rental properties show the same periods as net rental yield. **Correction** is excluded from return calculations.
