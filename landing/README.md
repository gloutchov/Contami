...
```powershell
ffmpeg -hide_banner -loglevel error -y -i landing\assets\example.gif -vf "fps=20,scale=1280:-2:flags=lanczos" -c:v libx264 -preset veryfast -crf 27 -pix_fmt yuv420p -movflags +faststart -an landing\assets\example.mp4
```

## Local preview

Start the dependency-free static server, which exposes only `landing/`:
