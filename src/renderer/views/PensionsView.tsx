...
export function PensionsView({ data, onSave }: { data: FinanceData; onSave: (command: FinanceCommand) => Promise<void> }) {
  ...
  return <>
    ...
    <section className="view-kpi-grid"><KpiCard label={t("pensionValue")} value={formatCurrency(totalValue, language)} icon={ShieldCheck} tone="gold"/><KpiCard label={t("pensionContributions")} value={formatCurrency(contributions, language)} icon={ArrowUpRight} tone="mint"/><KpiCard label={t("pensionLiquidations")} value={formatCurrency(withdrawals, language)} icon={ArrowDownRight} tone="coral"/></section>
    {plans.length ? plans.map((plan) => {
      const children = pensionCompartments(data, plan.id);
      return <section className="pension-group" key={plan.id}>
        <article className="panel entity-card clickable pension-collector-card" onClick={() => setSelected(plan)}>
          ...
          <div className="pension-collector-summary"><div className="entity-value">{formatCurrency(investmentPositionValue(data, plan), language, plan.currency)}</div><span><small>{t("compartments")}</small><strong>{children.length}</strong></span></div>
        ...
        {children.length ? <div className="entity-grid">{children.map((item) => <article className="panel entity-card clickable component-card" key={item.id} onClick={() => setSelected(item)}><header><div><h3>{item.name}</h3><p className="meta">{t("compartment")}{item.provider ? ` · ${item.provider}` : ""}</p></div><span className="pill">{item.active ? t("active") : t("closed")}</span></header><div className="entity-value">{formatCurrency(investmentPositionValue(data, item), language, item.currency)}</div><footer><span>{t("openDetails")}</span><div className="entity-actions" onClick={(event) => event.stopPropagation()}>{item.active && <button className="text-button" onClick={() => openEntry(item.id)}>{t("newInvestmentEntry")}</button>}<button className="text-button" onClick={() => toggleActive(item)}>{item.active ? t("close") : t("reopen")}</button><button className="icon-button" aria-label={t("edit")} onClick={() => openCompartment(plan.id, item)}><Pencil size={15}/></button><button className="icon-button danger" aria-label={t("delete")} onClick={() => remove("investment", item.id)}><Trash2 size={15}/></button></div></footer></article>)}</div> : <div className="panel pension-empty-compartments"><p>{t("noCompartments")}</p><button className="secondary-button" onClick={() => openCompartment(plan.id)}>{t("createCompartment")}</button></div>}
      </section>;
