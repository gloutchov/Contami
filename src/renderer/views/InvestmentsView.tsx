...
export function InvestmentsView({ data, onSave }: { data: FinanceData; onSave: (command: FinanceCommand) => Promise<void> }) {
  ...
  const investmentCard = (item: Investment, parent?: Investment) => {
    ...
    return <article className={`panel entity-card clickable${parent ? " component-card" : ""}`} key={item.id} onClick={() => openDetails(item)}><header><div><h3>{item.name}</h3><p className="meta">{typeName(item)}{parent ? ` · ${t("investmentGroup")}: ${parent.name}` : item.provider ? ` · ${item.provider}` : ""}</p></div><span className="pill">{item.active ? t("active") : t("closed")}</span></header><InvestmentCurrentValue value={valueOf(item)} currency={item.currency} isLoss={investmentPositionIsLoss(data, item)} trend={children.length ? undefined : investmentValuationTrend(data, item.id)} /><InvestmentMovementFacts totals={investmentPositionMovementTotals(data, item)} currency={item.currency} /><ReturnChart compact period="annual" series={returns} /><footer><span>{children.length ? t("groupTotal") : t("openDetails")}</span><div className="entity-actions" onClick={(event) => event.stopPropagation()}>{!children.length && <button className="text-button" onClick={() => openCorrection(item.id)}>{t("investmentCorrection")}</button>}{item.active && !children.length && <button className="text-button" onClick={() => openEntry(item.id)}>{t("newInvestmentEntry")}</button>}<button className="text-button" onClick={() => runUiAction(() => onSave({ type: "setActive", entity: "investment", id: item.id, active: !item.active, closedAt: item.active ? todayIso() : undefined }))}>{item.active ? t("close") : t("reopen")}</button><button className="icon-button" aria-label={t("edit")} onClick={() => setEditingInvestment(item)}><Pencil size={15}/></button><button className="icon-button danger" aria-label={t("delete")} onClick={() => remove("investment", item.id)}><Trash2 size={15}/></button></div></footer></article>;
  };
