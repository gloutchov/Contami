...
export function SharedExpenseForm({ data, value, onClose, onSave }: { data: FinanceData; value?: SharedExpense; onClose: () => void; onSave: (command: FinanceCommand) => Promise<void> }) {
  ...
  const valid = Boolean(description.trim() && Number(amount) > 0 && categoryId && paymentMethodId && sharesMatch);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const item: SharedExpense = { ...value, id: value?.id ?? crypto.randomUUID(), date, description, categoryId, paymentMethodId, amount: Number(amount), ownerShare: Number(ownerShare), partnerShare: Number(partnerShare), paidBy, settled: value?.settled ?? false, transactionId: value?.transactionId, notes };
