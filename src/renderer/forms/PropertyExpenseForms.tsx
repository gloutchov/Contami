...
type TaxKind = Extract<DetailKind, `tax_${string}`>;
...
const taxOptions: Array<{ value: TaxKind; label: TranslationKey; category: string }> = [
  { value: "tax_tv_licence", label: "tvLicence", category: "Canone TV" },
  { value: "tax_imu", label: "imu", category: "IMU" },
  { value: "tax_tari", label: "tari", category: "TARI" },
];
...
export function PropertyExpenseForm({ data, mode, value, initialPropertyId, onClose, onSave }: {
  ...
  const existingShared = existingTransaction
    ...
    : undefined;
  const options = mode === "utility" ? utilityOptions : taxOptions;
  const fallbackKind = options[0].value;
  const initialKind = value?.detailKind && options.some((item) => item.value === value.detailKind) ? value.detailKind : fallbackKind;
  ...
  const [taxInstallment, setTaxInstallment] = useState<NonNullable<PropertyEntry["taxInstallment"]>>(value?.taxInstallment ?? "single");
  ...
  const valid = Boolean(propertyId && description.trim() && Number(amount) > 0 && categoryId && paymentMethodId && (!shared || sharesMatch));
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!valid) return;
    const option = options.find((item) => item.value === detailKind) ?? options[0];
    ...
    const hasInstallments = detailKind === "tax_imu" || detailKind === "tax_tari";
    const entry: PropertyEntry = {
      ...
      taxInstallment: mode === "tax" && hasInstallments ? taxInstallment : undefined,
      paymentMethodId,
  ...
  return <Modal title={value ? t(mode === "utility" ? "editUtilityExpense" : "editPropertyTax") : t(mode === "utility" ? "newUtilityExpense" : "newPropertyTax")} onClose={onClose} onSubmit={submit} submitDisabled={!valid}>
    ...
    <Field label={t(mode === "utility" ? "utility" : "propertyTax")}><select value={detailKind} onChange={(event) => setDetailKind(event.target.value as DetailKind)}>{options.map((item) => <option key={item.value} value={item.value}>{t(item.label)}</option>)}</select></Field>
    {mode === "tax" && hasInstallments && <Field label={t("taxInstallment")}><select value={taxInstallment} onChange={(event) => setTaxInstallment(event.target.value as NonNullable<PropertyEntry["taxInstallment"]>)}><option value="single">{t("singlePayment")}</option><option value="first">{t("firstInstallment")}</option><option value="second">{t("secondInstallment")}</option></select></Field>}
