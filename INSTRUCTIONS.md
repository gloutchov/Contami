...
Every card contains a compact **annual return** chart; in the detail, the **monthly percentage return** appears below the invested-amount/countervalue chart. ContaMì uses Modified Dietz to separate performance from confirmed Contributions and Liquidations; Corrections and planned rows are excluded. The tooltip shows opening value, ending value, net flows, and weighted base. A month without sufficient valuation coverage remains blank. Complete years geometrically link their monthly returns, while annual summaries without detailed dates produce only a labelled **estimate** that assumes mid-period flows. A first historical year with no opening value is never invented.
