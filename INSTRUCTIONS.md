...
Every card contains a compact **annual return** chart; in the detail, the **monthly percentage return** appears below the invested-amount/countervalue chart. Each point compares the month’s latest confirmed valuation with the observation from the previous month and applies Modified Dietz over the actual interval between those dates, separating performance from intervening Contributions and Liquidations; Corrections and planned rows are excluded. The tooltip shows observation dates, opening value, ending value, net flows, and weighted base. A month without a valuation remains blank, and the next interval spanning multiple months is marked partial. Complete years geometrically link twelve covered intervals; summaries from two consecutive years instead produce a labelled **estimate** from prior and current closing values, assuming mid-period flows. A first year with no opening value is never invented.
